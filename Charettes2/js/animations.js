...
function wiggle_h(clicked_cell) {
    if (clicked_cell != "all") {
        ...
        // affect all
        $(".cell").each(function () {
            if (!$(this).hasClass("wiggle-h")) {
                $(this).addClass("wiggle-h");
                // then, after .7 seconds, remove the class
                setTimeout(function () {
                    $(".cell").removeClass("wiggle-h");
                }
                , 700);
            }
        });
...
function pulse_red(clicked_cell) {
    if (clicked_cell != "all") {
        if (!$(clicked_cell).hasClass("red")) {
            $(clicked_cell).addClass("red");
            // then, after 1 seconds, remove the class
            setTimeout(function () {
                $(clicked_cell).removeClass("red");
            }
            , 700);
        }
    } else {
...
function pulse_orange(clicked_cell) {
    if (clicked_cell != "all") {
        if (!$(clicked_cell).hasClass("orange")) {
            $(clicked_cell).addClass("orange");
            // then, after 1 seconds, remove the class
            setTimeout(function () {
                $(clicked_cell).removeClass("orange");
            }
            , 1000);
        }
    } else {
...
function tada(clicked_cell) {
    if (clicked_cell != "all") {
        ...
        let delay = 0;
        // affect all with increasing delay
        $(".cell").each(function () {
            setTimeout(() => {
        ...
        // after the total delay, remove the class
        setTimeout(function () {
            $(".cell").removeClass("tada");
        }
        , delay + 1000);
...
function wiggle_v(clicked_cell) {
    if (clicked_cell != "all") {
        ...
        // affect all
        $(".cell").each(function () {
            if (!$(this).hasClass("wiggle-v")) {
                $(this).addClass("wiggle-v");
                // then, after 1 seconds, remove the class
                setTimeout(function () {
                    $(".cell").removeClass("wiggle-v");
                }
                , 1000);
            }
        });
...
function fill_page(clicked_cell) {
    if (clicked_cell != "all") {
        ...
        // affect all
        $(".cell").each(function () {
            if (!$(this).hasClass("fill-page")) {
                $(this).addClass("fill-page");
                // then, after 1 seconds, remove the class
                setTimeout(function () {
                    $(".cell").removeClass("fill-page");
                }
                , 7000);
            }
        });
